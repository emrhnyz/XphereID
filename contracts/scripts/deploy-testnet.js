 ...
 *
 * Usage (from /contracts, after filling .env):
 *   npx hardhat run scripts/deploy-testnet.js --network xphereTestnet
 *
 * Does not ask for / print your private key.
 */
...
async function main() {
  ...
  const deployment = {
    ...
    rpc: process.env.RPC_TESTNET || "https://rpc.ankr.com/xphere_testnet",
    explorer: "https://xpt.tamsa.io",
    deployer: deployer.address,
    treasury,
    registerPriceWei: price.toString(),
    registerPriceXpt: ethers.formatEther(price),
    tld: ".xp",
    baseNode,
    contracts: {
  ...
  const outFile = path.join(outDir, "testnet.json");
  fs.writeFileSync(outFile, JSON.stringify(deployment, null, 2) + "\n");
  console.log("\nWrote", outFile);
  console.log(JSON.stringify(deployment.contracts, null, 2));
  console.log("\nDone. Update root DEPLOY_TESTNET.md addresses if needed.");
}
